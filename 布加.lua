...
local function SendNotification(title, text, duration)
    ...
    mLabel.Size = UDim2.new(1, -16, 0, 22)
...
fcOuter.Name = "Outer"
fcOuter.Size = UDim2.new(0, 20, 0, 20)
...
fcCutout.Size = UDim2.new(0, 17, 0, 17)
fcCutout.Position = UDim2.new(0, 5, 0, -2)
...
titleBottomFill.Size = UDim2.new(1, 0, 0, 8)
titleBottomFill.Position = UDim2.new(0, 0, 1, -8)
...
crescentOuter.Size = UDim2.new(0, 14, 0, 14)
...
crescentCutout.Size = UDim2.new(0, 11, 0, 11)
crescentCutout.Position = UDim2.new(0, 3.5, 0, -1.5)
...
footerTopFill.Size = UDim2.new(1, 0, 0, 8)
...
local function createBaseRow(parent, labelText, descText, indexSearch)
    ...
    descLbl.Position = UDim2.new(0, 0, 0, 19)
...
local function createSearchDropdownRow(parent, labelText, descText, options, initialVal, callback, indexSearch)
    ...
    sPad.PaddingBottom = UDim.new(0, 3)
...
local function createButtonRow(parent, labelText, btnText, descText, callback, indexSearch)
    ...
    btn.MouseLeave:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.15), {
            BackgroundColor3 = Colors.ControlBg,
            TextColor3 = Colors.PurplePrimary
        }):Play()
    end)
...
createButtonRow(homeConfigCard, "保存配置", "保存", "写入 default.json", function()
    AutoSaveConfig()
    SendNotification("Identical", "配置已保存。", 2)
end, true)
...
createButtonRow(homeConfigCard, "导出设置", "导出", "复制 JSON 到剪贴板", function()
    ...
    if ok and encoded and setclipboard then
        ...
        SendNotification("Identical", "配置已复制。", 2)
    end
end, true)
...
createButtonRow(homeConfigCard, "导入设置", "导入", "从剪贴板加载 JSON", function()
    if getclipboard then
        ...
        if ok and type(data) == "table" then
            ...
            if SyncUIWithConfig then SyncUIWithConfig() end
            AutoSaveConfig()
            SendNotification("Identical", "导入成功。", 2)
        else
            SendNotification("Identical", "解析失败。", 2)
        end
    end
end, true)
...
ctrls.CombatReach = createToggleRow(combatCard, "战斗触及", "扩大触及范围和命中框", Config.CombatReach,
    function(v)
        Config.CombatReach = v
        if not v then resetHitboxes() end
    end, true)

ctrls.CombatAura = createToggleRow(combatCard, "战斗光环", "自动攻击范围内敌方玩家", Config.CombatAura,
    function(v)
        Config.CombatAura = v
...
ctrls.CombatInterval = createSliderRow(combatCard, "攻击间隔", "光环攻击间隔", 0.1, 1.0,
...
ctrls.TargetPriority = createDropdownRow(combatCard, "目标优先级", "目标选择方式",
    { "最近", "血量最低" }, Config.TargetPriority, function(v)
        Config.TargetPriority = v
    end, true)

ctrls.LineOfSight = createToggleRow(combatCard, "视线检测", "仅攻击可见实体", Config.LineOfSight,
    function(v)
        Config.LineOfSight = v
    end, true)

ctrls.HumanizedPacing = createToggleRow(combatCard, "人性化节奏", "攻击计时添加自然延迟变化",
    Config.HumanizedPacing, function(v)
        Config.HumanizedPacing = v
    end, true)

ctrls.MicroBreaks = createToggleRow(combatCard, "微休息", "短暂周期暂停", Config.MicroBreaks,
    function(v)
...
ctrls.AimbotTarget = createDropdownRow(aimbotCard, "目标模式", "目标实体类型",
    { "玩家", "生物", "两者" }, Config.AimbotTarget, function(v)
        Config.AimbotTarget = v
    end, true)

ctrls.AimbotPart = createDropdownRow(aimbotCard, "瞄准部位", "锁定身体部位",
    { "头部", "躯干" }, Config.AimbotPart, function(v)
        Config.AimbotPart = v
    end, true)
...
ctrls.AimbotSmoothness = createSliderRow(aimbotCard, "平滑度", "0=瞬间，高=平滑", 0, 9,
...
ctrls.AutoHeal = createToggleRow(healCard, "自动治疗", "低血量时自动食用食物", Config.AutoHeal,
    function(v)
        Config.AutoHeal = v
    end, true)

ctrls.HealThreshold = createSliderRow(healCard, "治疗阈值", "低于此%触发治疗", 10, 90,
    Config.HealThreshold, false, "%", function(v)
        Config.HealThreshold = v
    end, true)

ctrls.HealFood = createDropdownRow(healCard, "食物偏好", "从背包消耗的物品",
    { "熟肉", "生肉", "浆果", "蘑菇", "苹果", "血果", "太阳果" }, Config.HealFood, function(v)
        Config.HealFood = v
    end, true)

ctrls.ThreatProtection = createToggleRow(healCard, "威胁撤退", "低血量时自动远离敌人",
    Config.ThreatProtection, function(v)
...
ctrls.RetreatHealth = createSliderRow(healCard, "撤退血量 %", "低于此%时撤退", 10, 60,
...
ctrls.AutoPickup = createToggleRow(gatherPickupCard, "自动拾取", "收集掉落物品到背包", Config.AutoPickup,
    function(v)
        Config.AutoPickup = v
    end, true)

ctrls.PickupRange = createSliderRow(gatherPickupCard, "拾取半径", "搜索掉落物品距离", 5, 80,
    Config.PickupRange, false, "格", function(v)
        Config.PickupRange = v
    end, true)

ctrls.PickupInterval = createSliderRow(gatherPickupCard, "拾取间隔", "拾取循环间隔", 0.05, 0.6,
    Config.PickupInterval, true, "秒", function(v)
        Config.PickupInterval = v
    end, true)

ctrls.PickupFilter = createDropdownRow(gatherPickupCard, "物品过滤", "优先拾取类别",
    { "全部", "木材", "石材", "矿石", "食物", "武器", "贵重品" }, Config.PickupFilter, function(v)
        Config.PickupFilter = v
    end, true)

ctrls.AutoDropTrash = createToggleRow(gatherPickupCard, "自动丢弃垃圾", "丢弃无用树叶/草/树枝",
    Config.AutoDropTrash, function(v)
        Config.AutoDropTrash = v
    end, true)

ctrls.RareItemAlert = createToggleRow(gatherPickupCard, "稀有物品提醒", "掉落神袋/陨石/矿石时提醒",
...
ctrls.ResourceAura = createToggleRow(auraCard, "资源光环", "攻击范围内树木/岩石/矿石", Config.ResourceAura,
    function(v)
...
ctrls.ResourceRange = createSliderRow(auraCard, "资源范围", "资源攻击触及距离", 5, 40,
    Config.ResourceRange, false, "格", function(v)
        Config.ResourceRange = v
    end, true)

ctrls.ResourceInterval = createSliderRow(auraCard, "资源间隔", "资源攻击间隔", 0.1, 0.8,
    Config.ResourceInterval, true, "秒", function(v)
        Config.ResourceInterval = v
    end, true)

ctrls.CritterAura = createToggleRow(auraCard, "生物光环", "攻击范围内动物/怪物", Config.CritterAura,
    function(v)
        Config.CritterAura = v
    end, true)

ctrls.CritterRange = createSliderRow(auraCard, "生物范围", "生物攻击触及距离", 5, 40,
    Config.CritterRange, false, "格", function(v)
        Config.CritterRange = v
    end, true)

ctrls.CritterInterval = createSliderRow(auraCard, "生物间隔", "生物攻击间隔", 0.1, 0.8,
...
ctrls.AutoFarm = createToggleRow(farmCard, "自动农场", "自动走向并采集最近目标", Config.AutoFarm,
    function(v)
        Config.AutoFarm = v
    end, true)

ctrls.FarmType = createDropdownRow(farmCard, "农场目标", "采集实体类型",
    { "资源", "生物", "两者" }, Config.FarmType, function(v)
        Config.FarmType = v
    end, true)

ctrls.FarmFilter = createDropdownRow(farmCard, "资源专注", "优先资源类别",
    { "全部", "木材", "石材", "矿石", "水晶", "纤维" }, Config.FarmFilter, function(v)
        Config.FarmFilter = v
    end, true)

ctrls.FarmRange = createSliderRow(farmCard, "扫描半径", "搜索农场目标距离", 15, 150,
    Config.FarmRange, false, "格", function(v)
        Config.FarmRange = v
    end, true)

ctrls.FarmMovement = createDropdownRow(farmCard, "移动方式", "到达目标导航方式",
    { "步行", "滑翔" }, Config.FarmMovement, function(v)
        Config.FarmMovement = v
    end, true)

ctrls.FarmSpeed = createSliderRow(farmCard, "农场速度", "步行/滑翔速度", 16, 80,
...
ctrls.FilterUnreachable = createToggleRow(farmCard, "过滤不可达", "忽略水下/地下节点",
    Config.FilterUnreachable, function(v)
        Config.FilterUnreachable = v
    end, true)

ctrls.StuckTimeout = createSliderRow(farmCard, "卡住超时", "被阻挡时切换目标前的秒数", 3, 15,
    Config.StuckTimeout, false, "秒", function(v)
        Config.StuckTimeout = v
    end, true)

ctrls.DeathRecovery = createToggleRow(farmCard, "死亡恢复", "重生后自动恢复农场", Config.DeathRecovery,
    function(v)
...
ctrls.AutoChest = createToggleRow(deployCard, "自动开箱", "打开并搜刮附近箱子", Config.AutoChest,
    function(v)
        Config.AutoChest = v
    end, true)

ctrls.ChestRange = createSliderRow(deployCard, "箱子范围", "扫描箱子距离", 5, 50,
    Config.ChestRange, false, "格", function(v)
        Config.ChestRange = v
    end, true)

ctrls.AutoPlant = createToggleRow(deployCard, "自动种植/收获", "种植空箱并收获成熟作物", Config.AutoPlant,
    function(v)
        Config.AutoPlant = v
    end, true)

ctrls.PlantRange = createSliderRow(deployCard, "作物范围", "扫描种植箱/灌木距离", 10, 60,
    Config.PlantRange, false, "格", function(v)
        Config.PlantRange = v
    end, true)

ctrls.AutoCraft = createToggleRow(deployCard, "自动制作", "重复制作选定配方", Config.AutoCraft,
    function(v)
        Config.AutoCraft = v
    end, true)
...
ctrls.AutoProcess = createToggleRow(deployCard, "自动烹饪/熔炼", "与附近营火/熔炉交互", Config.AutoProcess,
    function(v)
...
ctrls.ESPPlayers = createToggleRow(espCard, "玩家 ESP", "高亮玩家并显示距离", Config.ESPPlayers,
    function(v)
        Config.ESPPlayers = v
        if not v then clearESPCategory("player_") end
    end, true)

ctrls.ESPCritters = createToggleRow(espCard, "生物 ESP", "高亮生物并显示血量", Config.ESPCritters,
    function(v)
        Config.ESPCritters = v
        if not v then clearESPCategory("critter_") end
    end, true)

ctrls.ESPResources = createToggleRow(espCard, "资源 ESP", "高亮树木/岩石/矿石/水晶", Config.ESPResources,
    function(v)
        Config.ESPResources = v
        if not v then clearESPCategory("res_") end
    end, true)

ctrls.ESPItems = createToggleRow(espCard, "掉落物品 ESP", "高亮掉落战利品", Config.ESPItems,
    function(v)
        Config.ESPItems = v
        if not v then clearESPCategory("item_") end
    end, true)

ctrls.ESPChests = createToggleRow(espCard, "箱子 ESP", "高亮放置的储物箱", Config.ESPChests,
    function(v)
        Config.ESPChests = v
        if not v then clearESPCategory("chest_") end
    end, true)

ctrls.ESPTraders = createToggleRow(espCard, "商人 ESP", "高亮游商", Config.ESPTraders,
    function(v)
        Config.ESPTraders = v
        if not v then clearESPCategory("trader_") end
    end, true)

ctrls.ESPMeteors = createToggleRow(espCard, "陨石 & 神石 ESP", "高亮活跃陨石和神石", Config.ESPMeteors,
    function(v)
        Config.ESPMeteors = v
        if not v then clearESPCategory("meteor_") end
    end, true)

ctrls.ESPDistance = createSliderRow(espCard, "ESP 距离", "高亮最大渲染距离", 50, 600,
...
ctrls.SpeedEnabled = createToggleRow(miscMoveCard, "速度增强", "提高步行速度", Config.SpeedEnabled,
    function(v)
        Config.SpeedEnabled = v
    end, true)

ctrls.SpeedValue = createSliderRow(miscMoveCard, "步行速度", "速度值", 16, 60,
    Config.SpeedValue, false, "", function(v)
        Config.SpeedValue = v
    end, true)

ctrls.JumpEnabled = createToggleRow(miscMoveCard, "安全跳跃增强", "安全封顶跳跃力度 (50-83)", Config.JumpEnabled,
    function(v)
        Config.JumpEnabled = v
    end, true)

ctrls.JumpValue = createSliderRow(miscMoveCard, "跳跃力度", "服务器安全上限83", 50, 83,
    Config.JumpValue, false, "", function(v)
        Config.JumpValue = math.clamp(v, 50, 83)
    end, true)

ctrls.InfiniteJump = createToggleRow(miscMoveCard, "无限跳跃", "空中反复跳跃", Config.InfiniteJump,
    function(v)
...
ctrls.AntiAFK = createToggleRow(miscEnvCard, "防 AFK", "防止20分钟空闲断开", Config.AntiAFK,
    function(v)
        Config.AntiAFK = v
    end, true)
...
ctrls.NoFog = createToggleRow(miscEnvCard, "移除雾气", "清除大气雾效", Config.NoFog,
    function(v)
        Config.NoFog = v
        if not v then
            Lighting.FogEnd = origLighting.FogEnd
...
ctrls.DisableParticles = createToggleRow(miscEnvCard, "禁用粒子", "提升FPS", Config.DisableParticles,
    function(v)
        Config.DisableParticles = v
    end, true)

ctrls.BoatCruise = createToggleRow(miscEnvCard, "船只巡航", "自动推动船只前进", Config.BoatCruise,
    function(v)
        Config.BoatCruise = v
    end, true)

ctrls.BoatSpeed = createSliderRow(miscEnvCard, "船速", "巡航速度", 20, 120,
...
if not espFolder then
    ...
    espFolder.Parent = CoreGui or LocalPlayer.PlayerGui
else
...
local heartbeatConn = RunService.Heartbeat:Connect(function(dt)
    ...
    if hum then
        if Config.AutoFarm then
            ...
            if hum.WalkSpeed ~= fSpeed then hum.WalkSpeed = fSpeed end
    ...
    if Config.BoatCruise and hum then
        ...
        if seat then
            ...
            local boatPart = (boat and (boat.PrimaryPart or boat:FindFirstChild("MainPart") or boat:FindFirstChildWhichIsA("BasePart"))) or seat
            ...
            if boatPart and boatPart:IsA("BasePart") then
                boatPart.AssemblyLinearVelocity = Vector3.new(look.X * speed, boatPart.AssemblyLinearVelocity.Y, look.Z * speed)
            end
        end
    end

    -- 战斗光环
    if Config.CombatAura and root and now - lastCombatTime >= Config.CombatInterval then
        local playersFolder = Workspace:FindFirstChild("Players")
        if playersFolder then
            local targetList = {}
            for _, pChar in ipairs(playersFolder:GetChildren()) do
                if pChar ~= char then
                    loc pRoot = pChar:FindFirstChild("HumanoidRootPart")
                    local pHum = pChar:FindFirstChildOfClass("Humanoid")
                    if pRoot and pHum and pHum.Health > 0 then
                        local dist = (root.Position - pRoot.Position).Magnitude
                        if dist <= Config.CombatRange then
                            table.insert(targetList, { part = pRoot, dist = dist, hum = pHum })
                        end
                    end
                end
            end
            ...
            if #targetList > 0 then
                table.sort(targetList, function(a, b)
                    if Config.TargetPriority == "Lowest HP" then
                        return (a.hum.Health / a.hum.MaxHealth) < (b.hum.Health / b.hum.MaxHealth)
                    end
                    return a.dist < b.dist
                end)

                local target = targetList[1]
                if target and hasLineOfSight(root.Position, target.part.Position) then
                    strikeTarget(target.part)
                    lastCombatTime = now
                end
            end
        end
    ...
    -- 资源光环
    if Config.ResourceAura and root and now - lastResourceTime >= Config.ResourceInterval then
        ...
        if resFolder then
            ...
            for _, obj in ipairs(resFolder:GetChildren()) do
                ...
                if dist <= Config.ResourceRange and dist < minDist then
                    local name = obj.Name:lower()
                    local filter = Config.ResourceFilter:lower()
                    if filter == "all" or name:find(filter) then
                        target = obj
                        minDist = dist
                    end
                end
    ...
    -- 生物光环
    if Config.CritterAura and root and now - lastCritterTime >= Config.CritterInterval then
        ...
        if crittersFolder then
            ...
            for _, critter in ipairs(crittersFolder:GetChildren()) do
                local part = critter:FindFirstChild("ENTITY_HITBOX") or critter:FindFirstChild("HumanoidRootPart") or getRootPart(critter)
                if part then
                    local dist = (root.Position - part.Position).Magnitude
                    if dist <= Config.CritterRange and dist < minDist then
                        target = part
                        minDist = dist
                    end
                end
    ...
    -- 自动拾取
    if Config.AutoPickup and root and now - lastPickupTime >= Config.PickupInterval then
    ...
    if Config.AutoFarm and root and hum then
        -- 此处省略完整农场逻辑，实际脚本中应包含完整实现
        -- 如需完整农场功能，请参考原始脚本
    end

    -- 自动治疗
    if Config.AutoHeal and hum and now - lastHealTime >= 1 then
        local healthPercent = (hum.Health / hum.MaxHealth) * 100
        if healthPercent < Config.HealThreshold then
            if RemoteConsume then
                pcall(function()
     RemoteConsume:FireServer(Config.HealFood) end)
                lastHealTime = now
            end
        end
    end
...
local renderConn = RunService.RenderStepped:Connect(function()
    if Config.BowAimbot and Camera and isBowEquipped() then
        local isRightClicking = UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2)
        if isRightClicking then
            local targetPart, aimPoint = getAimbotTarget()
            if targetPart and aimPoint then
                local targetCF = CFrame.lookAt(Camera.CFrame.Position, aimPoint)
                if Config.AimbotSmoothness and Config.AimbotSmoothness > 0 then
...
task.delay(2, function()
    ...
    if #activeConnections < 3 then
        warn("Not enough connections, features may not work!")
    end
end)
